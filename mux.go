...
func (mux *ServeMux) Handler(r *http.Request) (h Handler, pattern string) {
	if r.Method != "CONNECT" {
		if p := cleanPath(r.URL.Path); p != r.URL.Path {
			...
			url := *r.URL
			url.Path = p
			return OldHandler(http.RedirectHandler(url.String(), http.StatusMovedPermanently)), pattern
		}
	}
...
func (mux *ServeMux) handler(host, path string) (h Handler, pattern string) {
	...
	if h == nil {
		h, pattern = OldHandler(http.NotFoundHandler()), ""
	}
	return
}
...
func (mux *ServeMux) Handle(pattern string, handler Handler) {
	...
	if n > 0 && pattern[n-1] == '/' && !mux.m[pattern[0:n-1]].explicit {
		...
		mux.m[pattern[0:n-1]] = muxEntry{h: OldHandler(http.RedirectHandler(url.String(), http.StatusMovedPermanently)), pattern: pattern}
	}
}
